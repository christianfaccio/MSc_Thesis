'''
MAPPO (Multi-Agent PPO, Yu et al. 2022) with parameter sharing — CTDE.
...
This is single-agent PPO (CleanRL lineage) lifted to a homogeneous swarm:
    ...
    - The actor is DECENTRALIZED: it takes the agent's LOCAL observation
      (decentralized execution).
    - The critic is CENTRALIZED: it takes the GLOBAL state of the whole swarm
      (centralized training). This is the only difference from IPPO, where the
      critic also uses the local obs (see src/multi_agent/policy.py: MappoPolicy
      vs IppoPolicy, and the env's info["global_state"]).
...
The global state is per-ENV (shape 11·n_agents + 2), shared by every agent in
that env, so V(global_state) is identical across agents of the same env; their
advantages still differ through the per-agent rewards.
...
its agents are done, and normalizes local obs / global state / rewards with its
own RunningMeanStd (the gym.vector + NormalizeObservation/Reward wrapper stack
used in the single-agent code cannot handle a per-agent layout).

Usage (from root):
    - train       -> `python -m src.multi_agent.mappo`
...
def train(args):
    ...
    global_dim = int(np.array(envs[0].global_observation_space.shape).prod())
    n_actions = envs[0].action_space.n

    # Parameter-shared actor-critic; the actor uses the LOCAL obs (decentralized
    # execution), the critic uses the GLOBAL state (centralized training).
    ...
    def normalize_global(raw, update=True):
        '''raw: (num_envs, global_dim). Centralized critic input — one global
        state per env, normalized with its own running stats.'''
        if update:
    ...
    # Global state is per-env (shared by all agents in that env) -> no agent axis.
    ...
    # Reset every env; stack local obs to (num_envs, n_agents, local_dim) and the
    # global state to (num_envs, global_dim).
    ...
    for e, env in enumerate(envs):
        o, info = env.reset(seed=args.seed + e)
        raw_obs[e] = o
    ...
    for iteration in range(start_iteration, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Actor sees local obs; critic sees the global state (same for every
            # agent of an env), so expand it along the agent axis before flattening.
            with torch.no_grad():
        ...
        # Global state has no agent axis -> expand it per agent before flattening,
        # so each agent-step sample carries its env's (shared) global state.
        ...
        for epoch in range(args.update_epochs):
            with torch.no_grad():
                # Centralized critic: values come from the GLOBAL state.
